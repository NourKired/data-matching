...
def test_driver(embeddings_file, configuration=None):
    ...
    if test_type == "EQ":
        ...
        mem_results.res_dict = entity_resolution(
            embeddings_file, configuration, info_file=info_file
        )
    elif test_type == "SM":
...
def match_driver(embeddings_file, df, configuration):
    ...
    m_tuples = entity_resolution(
        embeddings_file, configuration, info_file=info_file, task="match"
    )
