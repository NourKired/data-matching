...
def log_params():
    ...
    if not os.path.exists(path):
        with open(path, "w") as fp:
            ...
            header = (
                list(params.par_dict.keys())
                + list(metrics_dict.keys())
                + list(mem_results.res_dict.keys())
            )
            writer.writerow(header)
            writer.writerow(
                list(params.par_dict.values())
                + list(metrics_dict.values())
                + list(mem_results.res_dict.values())
            )
    else:
        with open(
            "pipeline/logger/" + params.par_dict["output_file"] + ".results", "a"
        ) as fp:
            writer = csv.writer(fp, delimiter=",")
