...
def parse_args():
    ...
    parser.add_argument(
        "--n_top", default=5, type=int, help="Number of neighbors to choose from."
    )
    parser.add_argument(
        "--n_candidates",
        default=1,
        type=int,
        help="Number of candidates to choose among the n_top " "neighbors.",
    )
...
def build_similarity_structure(
    ...
    if strategy == "basic":
        ...
        for n in tqdm(nodes):
            ...
            most_similar[n] = candidates
            print(
                "\rBuilding similarity structure: {:0.1f} - {}/{} tuples".format(
                    c / len(nodes) * 100, c, len(nodes)
                ),
                end="",
            )
            c += 1
        ...
        with open(model_file, "r") as fp:
            ...
            for idx, line in enumerate(fp):
                k, v = line.rstrip().split(" ", maxsplit=1)
                vector = np.array(list(map(float, v.split(" "))), ndmin=1).astype(
                    "float32"
                )
                mat.append(vector)
                words.append(k)
    ...
    diff = t_end - t_start
    print(
        "# Time required to build sim struct: {:.2} seconds".format(
            diff.total_seconds()
        )
    )
...
def compare_ground_truth_only(most_similar, matches_file, n_items, n_top):
    ...
    if total_candidates < 1:
        ...
        precision = count_hit / total_candidates
    ...
    if golden_candidates < 1:
        golden_precision = 0
    else:
        golden_precision = count_hit / golden_candidates
    try:
    ...
    pd.DataFrame(matches).to_json("matches_ER.json")
    # print(s.format([_*100 for _ in ]))
    print(
        "\n# Correct: {}\n# Wrong: {}\n# Total items: {}\n# Total matches: {}".format(
            count_hit, count_miss, iteration_counter, len(matches)
        )
    )
    return result_dict
...
def entity_resolution(
    input_file: str, configuration: dict, task: str = "test", info_file: str = None
):
    t_start = dt.datetime.now()
    ...
    most_similar = build_similarity_structure(
        model_file,
        viable_lines,
        n_items,
        strategy,
        ...
        num_trees=configuration["num_trees"],
    )
    if task == "test":
        dict_result = compare_ground_truth_only(
            most_similar, matches_file, n_items, n_top
        )
    elif task == "match":
