    ...
    for filename in list_files:
        ...
        with open(filename, "r") as fp:
            ...
            for line in fp:
                ...
                if len(terms) == 0:
                    warnings.warn(
                        "Problem encountered while reading line {}".format(line)
                    )
                    continue
                terms = [str(_) for _ in terms]
                expected = terms[-1]
...
def embeddings_quality(embeddings_file, configuration):
    ...
    if configuration["training_algorithm"] == "fasttext":
        ...
        wv = models.KeyedVectors.load_word2vec_format(
            embeddings_file, unicode_errors="ignore"
        )
    sum_total = 0
    count_tests = 0
    result_col = {}
    ...
    print("test_file", os.listdir(test_dir))
    ...
    result_dict = dict(
        chain.from_iterable(d.items() for d in (result_row, result_col, result_con))
    )
