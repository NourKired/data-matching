...
def detect_similar_attributes(df1, df2, n_dim, filepath, model, tokenizer):
    ...
    if l is not None:
        (
            ...
            labels,
            pca,
        ) = l  # emb_data_input = vecteurs embdi des Données input tabulaire
        # / bert_data_input = vecteurs du modele des Données input tabulaire
        indexes_to_remove = np.random.randint(0, len(labels), size=0)

        selected_labels = [
            labels[i] for i in range(len(labels)) if i not in indexes_to_remove
        ]
        selected_emb_data_input = [
            emb_data_input[i]
            for i in range(len(emb_data_input))
            if i not in indexes_to_remove
        ]
        selected_bert_data_input = [
            bert_data_input[i]
            for i in range(len(bert_data_input))
            if i not in indexes_to_remove
        ]

        P, U1, U2, V1, V2, s1, s2 = Matrice_de_passage(
            A1=selected_bert_data_input, A2=selected_emb_data_input
        )
        k = 0
        ...
        for it in range((len(cols_) // p)):
            ...
            for labKG_td in cols_it:
                if labKG_td not in selected_labels:
                    ...
                    v_labKG_td = np.array(get_emb_model(model, tokenizer, labKG_td))
                    print(v_labKG_td.shape, "v_labKG_td")
                    M_labKG.append(v_labKG_td)
            M_labKG = np.array(M_labKG)
            print(M_labKG.shape, "M_labKG")
            ...
            for v_attr_ea, label_ea in zip(M_labKG_A2, cols_it):
                print("v_attr_ea", v_attr_ea.shape)
                cos_sim_values = cosine_similarity(v_attr_ea, selected_emb_data_input)[
                    0
                ]
                ...
                if (
                    label_ea not in selected_labels
                ):  # je ne veux pas le detecter (FP / FN)
...
def get_matches(
    ...
    b, y_true, y_pred, M_E_Emb, indexes_to_remove, M_E_MOD = detect_similar_attributes(
        df1, df2, n_dim, filepath, model, tokenizer
    )
    ...
    new_row = {
        "dataset": input_name_dataset,
        "walk_strategy": walk_strategy,
        "n_walk": n_walk,
        "dimension": n_dim,
        "window": window_size,
        "learning_methd": learning_method,
        "training_algorithms": training_algorithm,
        "F1_score": f1_Score,
        "recall": np.round(recall, 2),
        "precision": np.round(precision, 2),
        "indexes_to_remove": str(indexes_to_remove),
